...
const AppearanceSection: React.FC<{ theme: string; onThemeChange: (t: 'light' | 'dark') => void; variants: any }> = React.memo(({ theme, onThemeChange, variants }) => (
...
const DateFormattingSection: React.FC<{ 
  dateFormat: DateFormat; 
  dateSeparator: DateSeparator; 
  onFormatChange: (f: DateFormat) => void; 
  onSeparatorChange: (s: DateSeparator) => void;
  variants: any;
  dateFormats: any[];
  dateSeparators: any[];
}> = React.memo(({ dateFormat, dateSeparator, onFormatChange, onSeparatorChange, variants, dateFormats, dateSeparators }) => (
  <motion.section className="settings-section" variants={variants}>
    <div className="section-header">
        ...
        <div className="animated-options-grid" style={{ flexDirection: 'row' }}>
          {dateSeparators.map((s) => (
            <button 
              ...
              onClick={() => onSeparatorChange(s.value)}
...
const LanguageSection: React.FC<{ 
  ...
  variants: any;
  languages: any[];
}> = React.memo(({ language, onLanguageChange, variants, languages }) => (
  <motion.section className="settings-section" variants={variants}>
    <div className="section-header">
    ...
    <div className="settings-group">
      <div className="setting-item">
        <div className="select-container">
          <select 
            value={language} 
            onChange={(e) => onLanguageChange(e.target.value as 'en' | 'vi' | 'zh')}
...
const NotificationSection: React.FC<{ 
  ...
  variants: any; 
}> = React.memo(({ emailNotif, onEmailChange, desktopNotif, onDesktopChange, variants }) => (
  <motion.section className="settings-section" variants={variants}>
    <div className="section-header">
...
const SecuritySection: React.FC<{ variants: any }> = React.memo(({ variants }) => (
  <motion.section className="settings-section" variants={variants}>
    <div className="section-header">
...
export const SettingsPage: React.FC = () => {
  ...
  const dateFormats = React.useMemo(() => [
    { label: 'DD-MM-YYYY', value: 'DD-MM-YYYY' as DateFormat },
    { label: 'YYYY-MM-DD', value: 'YYYY-MM-DD' as DateFormat },
    { label: 'MM-DD-YYYY', value: 'MM-DD-YYYY' as DateFormat },
  ], []);

  const dateSeparators = React.useMemo(() => [
    { label: 'Dash (-)', value: '-' as DateSeparator },
    { label: 'Slash (/)', value: '/' as DateSeparator },
  ], []);
  ...
  const onThemeChange = React.useCallback((t: 'light' | 'dark') => dispatch(setTheme(t)), [dispatch]);
    ...
    <div className="settings-view">
      ...
      >
        ...
        <NotificationSection 
          ...
          onEmailChange={() => setEmailNotif(!emailNotif)}
          desktopNotif={desktopNotif}
          onDesktopChange={() => setDesktopNotif(!desktopNotif)}
          variants={itemVariants}
        />
        <SecuritySection variants={itemVariants} />
      </motion.div>
    </div>
  );
};
