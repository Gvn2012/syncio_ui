...
import { CallVisualizer } from '../CallVisualizer/CallVisualizer';
  ...
  return (
    <div className="active-call-bar">
      ...
      <div className="visualizer-container">
        <CallVisualizer stream={localStream || null} height={32} color="var(--primary-alpha-20)" />
        <div className="peers-grid">
          {Array.from(remoteStreams.entries()).map(([userId, stream]) => (
            <CallVisualizer key={userId} stream={stream} height={32} color="var(--bg-surface)" />
          ))}
        </div>
      </div>
